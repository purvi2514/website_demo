...
const login = async (req, res, next) => {
  try {
    ...
    console.info('[LOGIN] Attempt from', req.ip, 'email:', email);

    // Find user (admin and regular users are stored in the same model)
    const user = await User.findOne({ email: email.toLowerCase() });
    if (!user) {
    ...
    // Prepare user object for response: return full model except password
    const userObj = user.toObject({ getters: true, virtuals: false });
    if (userObj.password) delete userObj.password;

    console.info('[LOGIN] Successful for:', email, 'role:', user.role);
